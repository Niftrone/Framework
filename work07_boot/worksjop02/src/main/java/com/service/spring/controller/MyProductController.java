...
public class MyProductController {
	...
	public ModelAndView findProducts(String command, String word) throws Exception{
		...
		List<MyProduct> result;
		
		if(command.equals("findProductByName") && !word.isEmpty()) {
			result = myProductService.findProductByName(word);
		} else if(command.equals("findProductByMaker") && !word.isEmpty()) {
			result = myProductService.findProductByMaker(word);
		} else {
			result = myProductService.findProducts();
		}
